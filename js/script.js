...
document.addEventListener('DOMContentLoaded', () => {
	...
	addForm.addEventListener('submit', (event) => {
		...
		if (newFilm) {
			...
			if (favorite == true) {
				console.log(newFilm + ' - Это ваш любимый фильм!');
			}
	...
	const delButtons = movieList.querySelectorAll('.delete');
	...
	function createMovieList(films, parent) {
		...
		document.querySelectorAll('.delete').forEach((btn, i) => {
			btn.addEventListener('click', () => {
				...
				//каждый раз когда мы удаляем какой то элемент, у нас будет заново 
				//перестраиаться таблица с фильмами (рекурсия)
